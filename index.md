## Objetivos del Proyecto
* **Aprender la arquitectura:** Comprender cómo se comunican la capa de Datos, Negocio y Presentación.
* **Optimizar código:** Reemplazar bucles e iteraciones largas por consultas LINQ de una sola línea.
* **Dominar bases de datos:** Aplicar filtros, funciones de agregado y operaciones CRUD eficientemente.
  
## Índice del Manual
* 📖 [**Fase 1:** Introducción a LINQ](./01-introduccion.md)
* 🏗️ [**Fase 2:** Arquitectura en Capas y LINQ](./02-arquitectura-capas.md)
* 🔍 [**Fase 3:** Consultas Básicas (Where, Select, OrderBy,ect)](./03-consultas-basicas.md)
* 📊 [**Fase 4:** Funciones de Agregado (Sum, Count, Max, MIn, GroupBy)](./04-funciones-agregado.md)
* 💾 [**Fase 5:** Operaciones CRUD con LINQ](./05-crud-linq.md)
* 💼 [**Fase 6:** Casos de Estudio Reales](./06-casos-estudio.md)
* 📑 [**Fase 7:** Referencias y Bibliografía](./07-referencias.md)

## Tecnologías Utilizadas
* **Lenguaje:** C# (.NET Core)
* **Herramientas:** LINQ to Entities 
* **Arquitectura:** Diseño lógico en 4 Capas
* **Documentación:** GitHub Pages 
